...
export function normalizePhase(value, fallback = 'work') {
  ...
  const raw = Object.keys(source).length
    ? (source.phase ?? (source.warmup === true ? 'warmup' : ''))
    : value
...
export function modeForEntry(input, fallback = null) {
  ...
  const workSets = sets.filter(set => normalizePhase(set, 'work') === 'work')
...
export function normalizePlannedSet(input, defaults = {}) {
  ...
  const phase = normalizePhase(source, target.phase)
...
export function normalizeEntry(input, defaults = {}) {
  ...
  const phase = normalizePhase(source, target?.phase || defaults.phase)
