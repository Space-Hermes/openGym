...
export function addSetForEntry(entry = {}) {
  ...
  if (warmupOnly) {
    const lastWarmup = [...sets].reverse().find(set => phaseOf(set) === 'warmup')
  ...
  const lastWork = [...sets].reverse().find(set => phaseOf(set) === 'work')
...
export function applyWarmupConfigToEntry(entry = {}, config = {}, history = [], increment = 0) {
  const source = objectOf(entry)
  const sets = Array.isArray(source.sets) ? source.sets : []
  const oldWarmups = sets.filter(set => phaseOf(set) === 'warmup')
  const workSets = sets.filter(set => phaseOf(set) === 'work')
  const target = { ...objectOf(source.target), ...objectOf(config) }
  const normalizedWorkSets = workSets.map(set => ({ ...set, phase: 'work' }))
  const configuredWarmups = prependWarmupSets(target, normalizedWorkSets, history, increment)
    .filter(set => phaseOf(set) === 'warmup')
    .map((set, index) => oldWarmups[index]?.done === true
      ? { ...oldWarmups[index], phase: 'warmup' }
      : set)
  return {
    ...source,
    target,
    sets: [...configuredWarmups, ...normalizedWorkSets]
  }
}
...
export function applyWorkConfigToEntry(entry = {}, config = {}, history = [], increment = 0) {
  const source = objectOf(entry)
  const sets = Array.isArray(source.sets) ? source.sets : []
  const warmups = sets.filter(set => phaseOf(set) === 'warmup')
  const workSets = sets.filter(set => phaseOf(set) === 'work')
  const target = { ...objectOf(source.target), ...objectOf(config) }
  const normalizedWarmups = warmups.map(set => ({ ...set, phase: 'warmup' }))
  const count = Math.max(1, Math.round(Number(target.sets) || workSets.length || 3))
  const rebuilt = []
  workSets.forEach((set, i) => {
    if (i >= count) return
    if (set.done === true) { rebuilt.push(set); return }
    rebuilt.push(setForTarget(target, 'work', history, increment))
  })
  while (rebuilt.length < count) rebuilt.push(setForTarget(target, 'work', history, increment))
  return { ...source, target, sets: [...normalizedWarmups, ...rebuilt] }
}
...
export function isAmrapResult(entry = {}, setIndex) {
  ...
  if (phaseOf(entry.sets[setIndex]) !== 'work') return false
  let lastWork = -1
  entry.sets.forEach((set, index) => {
    if (phaseOf(set) === 'work') lastWork = index
  })
...
export function amrapResultFor(entry = {}) {
  ...
  const index = entry.sets.findLastIndex(set => phaseOf(set) === 'work')
...
export function hasWorkRows(entry = {}) {
  return Array.isArray(entry.sets) && entry.sets.some(set => phaseOf(set) === 'work')
}
...
export function workRowsForMode(entry = {}, mode = 'reps') {
  ...
  return (Array.isArray(source.sets) ? source.sets : [])
    .filter(set => phaseOf(set) === 'work' && modeForSet(set, target) === expectedMode)
}
