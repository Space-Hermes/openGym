...
import { storedFromKg } from '../lib/units.js'
...
export default function Home() {
  ...
  const bwPoints = S.bodyweight.slice(-30).map(b => ({ t: b.t || new Date(b.d).getTime(), y: storedFromKg(b.w, S.unit), d: b.d }))
    ...
    <div className="card">
      ...
      {bw ? <>
        <div className="row" style={{ gap: 8, alignItems: 'baseline' }}>
          <div className="big">{fmtNum(storedFromKg(bw.w, S.unit))} <span className="muted" style={{ fontSize: '1rem' }}>{S.unit}</span></div>
          ...
          {!!delta && (
            <span className="small row" style={{ gap: 2, fontWeight: 500, color: bwDeltaColor(delta, bw.w) }}>
              ...
              {fmtNum(storedFromKg(Math.abs(delta), S.unit))}
            </span>
          )}
        ...
        {S.targetW && (
          <div className="small row" style={{ color: 'var(--yellow)', marginTop: 4, gap: 5 }}>
            ...
            <span>{t('Goal')} {fmtNum(storedFromKg(S.targetW, S.unit))} {S.unit} · {Math.abs(S.targetW - bw.w) < 0.05 ? t('reached!') : t(S.targetW > bw.w ? '{0} to gain' : '{0} to lose', fmtNum(storedFromKg(Math.abs(S.targetW - bw.w), S.unit)) + ' ' + S.unit)}</span>
          </div>
        )}
        <div className="chart" style={{ marginTop: 8 }}><LineChart points={bwPoints} h={130} unit={S.unit} goal={storedFromKg(S.targetW, S.unit)} /></div>
